...
func WatchForTLSProfileChanges(ctx context.Context, restConfig *rest.Config, onChanged func()) error {
	...
	go watchAPIServer(ctx, configClient, apiserver.Spec.TLSSecurityProfile, onChanged)
	return nil
}

const watchReconnectInterval = 30 * time.Second

func watchAPIServer(ctx context.Context, configClient configclientset.Interface, current *configv1.TLSSecurityProfile, onChanged func()) {
	for {
		if ctx.Err() != nil {
			return
		}
