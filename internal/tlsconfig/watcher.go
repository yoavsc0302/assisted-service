...
// The watcher compares spec.tlsSecurityProfile to avoid unnecessary restarts
// on unrelated APIServer changes (e.g. status updates during node rollouts).
func WatchForTLSProfileChanges(ctx context.Context, restConfig *rest.Config, onChanged func()) error {
	...
	go watchAPIServer(ctx, configClient, apiserver.Spec.TLSSecurityProfile, apiserver.Spec.TLSAdherence, onChanged)
	return nil
}

const watchReconnectInterval = 30 * time.Second

func watchAPIServer(ctx context.Context, configClient configclientset.Interface, currentProfile *configv1.TLSSecurityProfile, currentAdherence configv1.TLSAdherencePolicy, onChanged func()) {
	for {
		if ctx.Err() != nil {
			return
		}
		...
		for event := range w.ResultChan() {
			...
			adherenceChanged := currentAdherence != updated.Spec.TLSAdherence
