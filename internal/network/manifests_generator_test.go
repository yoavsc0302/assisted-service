...
var _ = Describe("chrony manifest", func() {
	...
	Context("Create NTP Manifest", func() {
		...
		It("Uses only ntp_sources when set, no pool and no discovered sources", func() {
			...
			Expect(chronyConf).ToNot(ContainSubstring(common.TestNTPSourceSynced.SourceName))

			chronyConfServers := extractChronyConfServers(response)
			Expect(chronyConfServers).To(ConsistOf("my-ntp.corp.com"))
		})
